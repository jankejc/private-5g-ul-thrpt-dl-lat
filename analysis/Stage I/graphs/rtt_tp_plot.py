...
# === Main Script ===
throughput_df = pd.read_csv('averaged_throughput_by_configuration.csv')
rtt_df = pd.read_csv('ping_boxplot_stats.csv')
...
colors = cc.glasbey[1:25]  # 24 high-contrast, colorblind-friendly colors
...
for index, (name, coords) in enumerate(configurations.items()):
    ...
    ax.scatter(coords['x'], coords['y'], label=f"{index+1}. {name}",
               s=200, alpha=0.8, color=color)

    text = ax.text(coords['x'], coords['y'],
                   f"{index + 1}", fontsize=12,
                   ha='center', va='center',
...
ax.set_xlabel('Average Throughput [Mibps]', fontsize=18, weight='bold')
ax.set_ylabel('Average RTT [ms]', fontsize=18, weight='bold')
...
plt.tight_layout(rect=[0.02, 0.1, 1, 0.99])
