...
class ServiceRecvVxlanLinuxHost(VxlanLinuxHost):
    ...
    def run_vxlan_ping_test(self,
        ...
        try:
            ...
            if exit_status == 0:
                msg = f"Ping Packet Size {ping_size} completed successfully."
                if save_pcap:
                    msg += f" Pcap saved as {pcap_file}."
                print_success(msg)
                return True
            else:
                print_error(f"Ping for Packet Size {ping_size} failed: {stderr}")
                return False
        except Exception as e:
            print_error(f"Exception during Ping for Packet Size {ping_size}: {e}")
            return False
